...
@Component({
  selector: 'preferences-default-chat',
  templateUrl: './default-chat.page.html',
  styleUrls: ['./default-chat.page.scss'],
})
export class DefaultChatPage implements OnInit {

  myForm: FormGroup = {} as FormGroup  
  private chat$: Observable<any> = of(null)
  ...
  async preloadData() {
    ...
    this.state = 'loading'
    this.chat$ = await this.feed.getField(database.literal.default, {field: database.list.default.chat})
    this.chat$
      .pipe(
        tap(doc => {
          if (doc) {
    ...
    .pipe(
      ...
      tap(change => {
          if (this.myForm.valid && this.state === 'modified') {          
            this.feed.save(database.literal.default,{field: database.list.default.chat, ...this.myForm.value})  
          this.state = 'synced'         
  ...
  onSubmit(e:any) {
    if (this.myForm.valid && this.state ==='modified') {
      this.feed.save(database.literal.default,{field: database.list.default.chat,...e})    
      this.state = 'synced'
      this.presentToast('Sincronizado', 'success')
    } else 
