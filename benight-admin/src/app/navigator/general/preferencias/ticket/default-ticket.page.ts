...
@Component({
  selector: 'preferences-default-ticket',
  templateUrl: './default-ticket.page.html',
  styleUrls: ['./default-ticket.page.scss'],
})
export class DefaultTicketPage implements OnInit {

  myForm: FormGroup = {} as FormGroup  
  private ticket$: Observable<any> = of(null)
  ...
  async preloadData() {
    ...
    this.state = 'loading'
    this.ticket$ = await this.feed.getField(database.literal.default, {field: database.list.default.ticket})
    this.ticket$
      .pipe(
        tap(doc => {
          if (doc) {
    ...
    .pipe(
      ...
      tap(change => {
          if (this.myForm.valid && this.state === 'modified') {          
            this.feed.save(database.literal.default,{field: database.list.default.ticket, ...this.myForm.value})  
          this.state = 'synced'         
  ...
  onSubmit(e:any) {
    if (this.myForm.valid && this.state ==='modified') {
      this.feed.save(database.literal.default,{field: database.list.default.ticket, ...e})    
      this.state = 'synced'
      this.presentToast('Sincronizado', 'success')
    } else 
