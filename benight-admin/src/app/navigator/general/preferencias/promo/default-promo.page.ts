...
@Component({
  selector: 'preferences-default-promo',
  templateUrl: './default-promo.page.html',
  styleUrls: ['./default-promo.page.scss'],
})
export class DefaultPromoPage implements OnInit {

  myForm: FormGroup = {} as FormGroup  
  private promo$: Observable<any> = of(null)
  ...
  async preloadData() {
    ...
    this.state = 'loading'
    this.promo$ = await this.feed.getField(database.literal.default, { field: database.list.default.promo})
    this.promo$
      .pipe(
        tap(doc => {
          if (doc) {
    ...
    .pipe(
      ...
      tap(change => {
          if (this.myForm.valid && this.state === 'modified') {          
            this.feed.save(database.literal.default,{field: database.list.default.promo, ...this.myForm.value})  
          this.state = 'synced'         
  ...
  onSubmit(e:any) {
    if (this.myForm.valid && this.state ==='modified') {
      this.feed.save(database.literal.default,{field: database.list.default.promo, ...e})    
      this.state = 'synced'
      this.presentToast('Sincronizado', 'success')
    } else 
