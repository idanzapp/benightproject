...
@Component({
  selector: 'preferences-default-event',
  templateUrl: './default-event.page.html',
  styleUrls: ['./default-event.page.scss'],
})
export class DefaultEventPage implements OnInit {
  ...
  async preloadData() {
    //this.feed.copy(database.literal.default)
    await this.route.params.pipe(map(p => p.id)).subscribe(e => this.id = e)
    this.state = 'loading'
    this.event$ = await this.feed.getField(database.literal.default, {field: database.list.default.event})
    this.event$
      .pipe(
        tap(doc => {
          if (doc) {
    ...
    .pipe(
      ...
      tap(change => {
          if (this.myForm.valid && this.state === 'modified') {          
            this.feed.save(database.literal.default,{field: database.list.default.event, ...this.myForm.value})  
          this.state = 'synced'         
  ...
  onSubmit(e:any) {
    if (this.myForm.valid && this.state ==='modified') {
      this.feed.save(database.literal.default,{field: database.list.default.event, ...e})    
      this.state = 'synced'
      this.presentToast('Sincronizado', 'success')
    } else 
